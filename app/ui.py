"""
Zariya – Full UI (v2)
Multi-page Streamlit app: Chat, Flashcards, Notepad, Settings.
"""
...
def _safe(text: str) -> str:
    """Escape HTML in user content before injecting into HTML templates."""
    return html.escape(str(text))


def render_bubble(content: str, role: str, timestamp: str = ""):
    """Render a markdown-enabled chat bubble."""
    ...
    # Render content through st.markdown for proper markdown support
...
def _init_state():
    defaults = {
        ...
        "fc_flipped": set(),
        "fc_known": set(),
        "notes": [],          # list of {"id", "title", "body", "created_at"}
        "editing_note": None,
        "regen_trigger": None,
    }
...
# Update engine settings from session state
...
if page == "chat":
    ...
    <div class="page-header">
      ...
      <div>
        ...
        <div class="page-sub">ذریعہ · Urdu & English · Fully Offline</div>
      </div>
    ...
    with tc4:
        if st.button("✏️ Auto-Title", use_container_width=True):
    ...
    if not messages:
        ...
        <div class="empty">
          ...
          <div class="empty-sub">Your offline AI assistant. Ask in Urdu or English.</div>
    ...
    # Messages
        ...
        # Copy / Regen buttons on last assistant message
        last_bot = next((m for m in reversed(raw_msgs) if m["role"] == "assistant"), None)
        if last_bot:
            b1, b2, b3, b4, _ = st.columns([1.5, 1.5, 1.5, 1.5, 5])
            with b1:
                st.button("📋 Copy", key="copy_last",
                    help="Copy last response to clipboard",
                    on_click=lambda: None)
            with b2:
                if st.button("🔁 Regen", key="regen_last"):
            ...
            with b3:
                if tts.available:
                    if st.button("🔊 Read", key="tts_last"):
                        tts.speak(last_bot["content"])
            with b4:
                if st.button("📌 Pin", key="pin_sess",
                    help="Rename this conversation with AI"):
                    st.session_state.pending_input = "__AUTOTITLE__"
                    st.rerun()
    ...
    # Chat input
    user_input = st.chat_input("Kuch bhi poochho… Ask anything in Urdu or English")
    ...
    if user_input:
        ...
        if user_input == "__SUMMARIZE__":
            ...
            if ready:
                ...
                err = ("⚠️ **No model loaded.** Go to ⚙️ Settings for setup instructions.")
...
# ══════════════════════════════════════════════════════
    ...
    if cards:
        ...
        if len(known) == total:
            st.success("🎉 You've reviewed all cards! Generate a new set to continue studying.")
            ...
            with cols[i % 2]:
                ...
                with c1:
                    ...
                    if st.button(label, key=f"flip_{i}", use_container_width=True):
                        ...
                        st.rerun()
                with c2:
                    if st.button("✓ Know", key=f"know_{i}", use_container_width=True):
            ...
            if note:
                ...
                with ec2:
                    if st.button("📋 Copy", use_container_width=True):
                        st.code(new_body)  # shows in copyable block
                with ec3:
                    # Ask AI to improve/summarize note
...
# ══════════════════════════════════════════════════════
    ...
    <div class="page-header">
      ...
      <div>
        ...
        <div class="page-sub">Model, interface, TTS, and data</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
**Recommended models:**
- `Mistral-7B-Instruct-v0.2.Q4_K_M.gguf` (~4.4 GB) — best all-round
- `Meta-Llama-3-8B-Instruct.Q4_K_M.gguf` (~4.9 GB) — strong reasoning
- `phi-3-mini-4k-instruct.Q4_K_M.gguf` (~2.2 GB) — fast, low RAM
...
```
    ...
    with st.container():
        st.markdown('<div class="setting-group">', unsafe_allow_html=True)
        st.markdown('<div class="setting-title">🎛️ Generation</div>', unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
    ...
    with st.container():
        ...
        if tts.available:
            st.session_state.tts_enabled = st.toggle("Enable auto-read aloud", st.session_state.tts_enabled)
            ...
            new_rate = st.slider("Speed (words/min)", 80, 280, st.session_state.tts_rate, 10)
    ...
    with st.container():
        st.markdown('<div class="setting-group">', unsafe_allow_html=True)
        st.markdown('<div class="setting-title">🖥️ Interface</div>', unsafe_allow_html=True)
    ...
    with st.container():
        st.markdown('<div class="setting-group">', unsafe_allow_html=True)
        st.markdown('<div class="setting-title">ℹ️ About</div>', unsafe_allow_html=True)
        st.markdown("""
...
⚡ Token streaming · 💬 Session history · 🃏 Flashcards · 📝 Notepad

[GitHub](https://github.com/aayanahmed20/zariya) · MIT License © 2026 Aayan Ahmed
        """)
        st.markdown('</div>', unsafe_allow_html=True)
